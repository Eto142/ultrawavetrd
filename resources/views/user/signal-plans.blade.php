 ...
 Settings
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('user.signals') }}" class="px-4 py-2 rounded-lg bg-surface-overlay border border-surface-border text-content-secondary hover:text-content-primary text-sm font-medium transition-colors">Signals</a>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($plans as $plan)
            <div class="rounded-xl bg-surface-raised border {{ $plan->badge_label ? 'border-primary hover:border-primary shadow-primary/5 shadow-lg' : 'border-surface-border hover:border-primary/50' }} overflow-hidden transition-colors flex flex-col">
                ...
                <div class="p-6 text-center border-b border-surface-border">
                    ...
                    @if ($plan->badge_label)
                        <span class="inline-block mb-2 px-2.5 py-0.5 text-xs font-semibold rounded-full bg-primary/10 text-primary">{{ $plan->badge_label }}</span>
                    @endif
                    <div>
                        <span class="text-3xl font-bold text-primary">${{ number_format($plan->price, 0) }}</span>
                    </div>
                </div>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                            ...
                            <span class="text-content-secondary">{{ $feature }}</span>
                        </div>
                    @endforeach
                    <div class="flex items-center gap-2 text-sm">
    ...
    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        ...
                        <span class="text-content-secondary">Duration: {{ $plan->duration_days }} Days</span>
                    </div>
                </div>

                <div class="px-6 pb-6">
                    <form id="subscribe-form-{{ $plan->id }}" action="{{ route('user.signal-plans.subscribe') }}" method="POST">
                        @csrf
                        <input type="hidden" name="signal_plan_id" value="{{ $plan->id }}">
        ...
        @empty
            <div class="col-span-full rounded-xl bg-surface-raised border border-surface-border p-8 text-center text-content-tertiary">
                No signal plans are available right now.
            </div>
        @endforelse
    </div>
