 ...
 Settings
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-surface-border">
                            ...
                            <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Expires</th>
                            <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Subscribed</th>
                            <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-surface-border">
                        @foreach ($subscriptions as $subscription)
                            <tr>
                                <td class="px-5 py-3.5 text-content-primary font-medium">{{ $subscription->signalPlan->name ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-content-secondary">${{ number_format($subscription->amount, 2) }}</td>
                                <td class="px-5 py-3.5 text-content-secondary">{{ $subscription->payment_method }}</td>
                                <td class="px-5 py-3.5">
                                    @if ($subscription->isActive())
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gain/10 text-gain">Active</span>
                                    @elseif ($subscription->status === \App\Models\SignalSubscription::STATUS_APPROVED)
                                ...
                                <td class="px-5 py-3.5 text-content-tertiary">{{ $subscription->expires_at?->format('M d, Y') ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-content-tertiary">{{ $subscription->created_at->format('M d, Y') }}</td>
                                <td class="px-5 py-3.5 text-right">
                                    @if ($subscription->signalPlan)
                                        <form id="renew-form-{{ $subscription->id }}" action="{{ route('user.signal-plans.subscribe') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="signal_plan_id" value="{{ $subscription->signalPlan->id }}">
                                            <button type="button"
                                                    onclick="confirmRenewal({{ $subscription->id }}, {{ Illuminate\Support\Js::from($subscription->signalPlan->name) }}, {{ $subscription->signalPlan->price }})"
                                                    class="px-3 py-1.5 rounded-lg bg-surface-overlay border border-surface-border text-content-secondary hover:text-content-primary text-xs font-medium transition-colors">
                                                Renew
                                            </button>
                                        </form>
                                    @endif
    ...
    function confirmRenewal(subscriptionId, planName, price) {
        Swal.fire({
            ...
            text: 'Renew ' + planName + ' for $' + price.toLocaleString() + '?',
            icon: 'question',
