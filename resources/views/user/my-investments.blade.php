 ...
 Settings
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-content-secondary mt-1">Track the status of your investments</p>
        </div>
        <a href="{{ route('user.investment.plan') }}" class="px-4 py-2 rounded-lg bg-primary hover:bg-primary-dark text-content-inverse text-sm font-medium transition-colors">
            Invest Now
        </a>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-surface-border">
                        ...
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Duration</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Payment Method</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Status</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Invested</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-surface-border">
                    @forelse ($investments as $investment)
                        <tr>
                            <td class="px-5 py-3.5 text-content-primary font-medium">{{ $investment->investmentPlan->name ?? '—' }}</td>
                            <td class="px-5 py-3.5 text-content-secondary">${{ number_format($investment->amount, 2) }}</td>
                            <td class="px-5 py-3.5 text-content-secondary">{{ $investment->investmentPlan->duration_days ?? '—' }} Days</td>
                            <td class="px-5 py-3.5 text-content-secondary">{{ $investment->payment_method }}</td>
                            <td class="px-5 py-3.5">
                                @if ($investment->status === \App\Models\Investment::STATUS_APPROVED)
                            ...
                            <td class="px-5 py-3.5 text-content-tertiary">{{ $investment->created_at->format('M d, Y') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-content-tertiary">You haven't invested in any plans yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
