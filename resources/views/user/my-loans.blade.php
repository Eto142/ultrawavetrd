 ...
 Settings
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-content-secondary mt-1">Track the status of your loan applications</p>
        </div>
        <a href="{{ route('user.loans') }}" class="px-4 py-2 rounded-lg bg-primary hover:bg-primary-dark text-content-inverse text-sm font-medium transition-colors">
            Apply for Loan
        </a>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-surface-border">
                        ...
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Monthly Payment</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Total Repayable</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Status</th>
                        <th class="text-left text-xs font-medium text-content-tertiary uppercase tracking-wider px-5 py-3">Applied</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-surface-border">
                    @forelse ($loans as $loan)
                        <tr>
                            <td class="px-5 py-3.5 text-content-primary font-medium">{{ $loan->loanPlan->name ?? '—' }}</td>
                            ...
                            <td class="px-5 py-3.5 text-content-secondary">${{ number_format($loan->total_repayable, 2) }}</td>
                            <td class="px-5 py-3.5">
                                @if ($loan->status === \App\Models\Loan::STATUS_APPROVED)
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-8 text-center text-content-tertiary">You haven't applied for any loans yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
